...
function countMonths(ledger: Ledger): number {
  const ms =
    new Date(ledger.dateLast).getTime() - new Date(ledger.dateFirst).getTime();
      ...
      panels: [
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const lastValue =
              dataset.length > 0 ? dataset[dataset.length - 1].value : 0;
            return StatChart(
              dataset,
              TREND_POSITIVE,
              currencyFormatter(lastValue),
              COLOR_PROFIT
            );
          },
        },
            ...
            const results = await Promise.all(
              queries.map(async (query) => ({
                ...
                dataset: fillMonthlyDataset(
                  (
                    await ledger.query(query.bql)
                  ).map((row) => ({
                    date: `${row.year}-${row.month}`,
                    value:
                      query.stack === "income"
                        ? -row.value[variables.currency]
                        : row.value[variables.currency],
                  })),
                  "date",
                  iterateMonths(ledger.dateFirst, ledger.dateLast).map(
                    (m) => `${m.year}-${m.month}`
                  ),
                  { value: 0 }
                ),
      ...
      panels: [
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const data = result
              ...
              .map((row) => ({
                name: row.currency,
                value: row.market_value[variables.currency],
              }));
          ...
          spec: async ({ ledger, variables }) => {
            ...
            return {
              ...
              onClick: (event) => {
                ...
                const link = "../../balance_sheet/?time={time}".replace(
                  "{time}",
                  `${year}-${month.padStart(2, "0")}`
                );
                window.open(ledger.urlFor(link));
        ...
        },
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const dataset = result.map((row) => ({
              date: `${row.year}-${row.month}`,
              value:
                row.market_value[variables.currency] -
                row.book_value[variables.currency],
            }));
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const queries = await Promise.all(
              years.map((year) =>
                ledger.query(`SELECT currency,
                              CONVERT(SUM(position), '${
                                variables.currency
                              }', ${year}-12-31) as market_value
            ...
            for (let i = 0; i < years.length; i++) {
              ...
              for (const row of query) {
                ...
                const assetClass =
                  ledger.commodities[row.currency]?.meta.asset_class ??
                  "unknown";
                amounts[year][assetClass] =
                  (amounts[year][assetClass] ?? 0) + value;
            ...
            return {
              tooltip: {
                formatter: (params: any) => {
                  const sum = Object.values(amounts[params.name]).reduce(
                    (prev, cur) => prev + cur,
                    0
                  );
        ...
        },
        {
          ...
          spec: async ({ ledger, variables }) => {
            const currencyFormatter = getCurrencyFormatter(variables.currency);
            const filter =
              variables.categories == "Only investments"
                ? `AND currency != '${variables.currency}'`
                : "";
            const result = await ledger.query(
              `SELECT currency, CONVERT(SUM(position), '${variables.currency}') as market_value
            ...
            for (const row of result) {
              ...
              const value = row.market_value[variables.currency];
              const assetName = (ledger.commodities[ccy]?.meta.name ??
                ccy) as string;
              const assetClass = (ledger.commodities[ccy]?.meta.asset_class ??
                "unknown") as string;
              ...
              tooltip: {
                formatter: (params: any) =>
                  `${params.marker} ${
                    ...
                    params.value
                  )}</span> (${((params.value / totalValue) * 100).toFixed(
                    0
                  )}%)`,
              },
              series: [
          ...
          spec: async ({ ledger, variables }) => {
            ...
            for (const row of result) {
              ...
              const allocations = Object.entries(
                ledger.commodities[row.currency]?.meta ?? {}
              ).filter(([k, _v]) => k.startsWith("asset_allocation_")) as [
                string,
                number
              ][];
              ...
              for (const [allocation, percentage] of allocations) {
                const parts = allocation
                  .substring("asset_allocation_".length)
                  .split("_");
                let node = root;
                ...
                const value =
                  (percentage / 100) * row.market_value[variables.currency];
      ...
      panels: [
        ...
        },
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const avgSavingsRatePercent =
              avgIncome === 0 ? 0 : 1 - avgExpenses / avgIncome;
            const trend_color =
              avgSavingsRate >= 0 ? TREND_POSITIVE : TREND_NEGATIVE;

            const incomeMap = new Map<string, number>(
              incomeDataset.map(({ date, value }) => [date, value])
            );
            const expensesMap = new Map<string, number>(
              expensesDataset.map(({ date, value }) => [date, value])
            );
            ...
            const text = `${currencyFormatter(
              avgSavingsRate
            )} (${percentFormatter(avgSavingsRatePercent)})`;
          ...
          spec: async ({ ledger, variables }) => {
            ...
            return {
              ...
              series: [
                {
                  ...
                  data: data.map(([label, value]) => {
                    ...
                    const monthLocale = monthFormatter(
                  ...
                  label: {
                    show: true,
                    formatter: (params: any) =>
                      currencyFormatter(params.data[2]),
                  },
                  emphasis: {
                    itemStyle: {
                      shadowBlur: 10,
              ...
              onClick: (event) => {
                ...
                const link = "../../income_statement/?time={time}".replace(
                  "{time}",
                  time
                );
                window.open(ledger.urlFor(link));
        ...
        },
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const clickEvt =
              window.screen.width < 800 ? "onDblClick" : "onClick";
            ...
            return {
              ...
              [clickEvt]: (event: ECElementEvent) => {
                const account =
                  "Income" +
                  event.treePathInfo.map((i: any) => i.name).join(":");
                const link = "../../account/{account}/?r=changes".replace(
                  "{account}",
                  account
                );
                window.open(ledger.urlFor(link));
              ...
              columns: [
                ...
                { field: "narration", flex: 1 },
                {
                  ...
                  valueFormatter: (_value, row) =>
                    `${row.position.units.number} ${row.position.units.currency}`,
                },
              ],
      ...
      panels: [
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            return {
              ...
              onClick: (event) => {
                const link =
                  "../../account/Expenses/?filter=#travel&time={time}".replace(
                    "{time}",
                    event.name
                  );
                window.open(ledger.urlFor(link));
        ...
        },
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            for (const row of result) {
              const tag =
                (row.tags as string[]).find((tag) => tag.match(/-\d{4}/)) ??
                "unknown";
              if (!(tag in amounts)) {
                travels.push(tag);
                amounts[tag] = 0;
            ...
            return {
              ...
              onClick: (event) => {
                const link = "../../account/Expenses/?filter=#{travel}".replace(
                  "{travel}",
                  event.name
                );
                window.open(ledger.urlFor(link));
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const savings =
              accountTree.children[0].name === "Income"
                ? -accountTree.children[0].value! -
                  accountTree.children[1].value!
                : -accountTree.children[1].value! -
                  accountTree.children[0].value!;
            if (savings > 0) {
            ...
            return {
              ...
              onClick: (_event, node) => {
                ...
                const link = "../../account/{account}/".replace(
                  "{account}",
                  node.name
                );
                window.open(ledger.urlFor(link));
      ...
      panels: [
        {
          ...
          spec: async ({ ledger, variables }) => {
            ...
            const finalAmount =
              result.length > 0 ? result.at(-1)!.value[variables.currency] : 0;
            ...
            const dateLast = new Date(
              new Date(resultExLast.year, resultExLast.month, 1).getTime() - 1
            );
            const days =
              (dateLast.getTime() - dateFirst.getTime()) /
                (1000 * 60 * 60 * 24) +
              1;
            const totalDiff =
              (resultExLast.value[variables.currency] ?? 0) -
              (resultEx[0].value[variables.currency] ?? 0);
            ...
            const months = iterateMonths(dateFirstStr, dateProjectUntilStr).map(
              (m) => `${m.year}-${m.month}`
            );
            const lastMonthIdx = months.findIndex(
              (m) => m === `${dateLastYear}-${dateLastMonth}`
            );
